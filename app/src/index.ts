...
import { User, UserCookie, TimeSlot, UserCredential, UserTimeSlotInfo } from "./index.d"
...
function getTimeSlotDate(slot: TimeSlot): Date {
...
async function main() {
    ...
    while (true) {
        ...
        console.log(`Checking again at ${toISOString(nextCheck)} (${dayNames[nextCheck.getDay()]}) ${nextCheck.getHours()}:${nextCheck.getMinutes() < 10 ? "0" : ""}${nextCheck.getMinutes()}`)
    }
